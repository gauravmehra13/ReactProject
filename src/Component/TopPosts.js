...
const TopPosts = () => {
  ...
  const max1 = bollywood.reduce((prev, current) =>
    prev.likes > current.likes ? prev : current
  );
  const max2 = hollywood.reduce((prev, current) =>
    prev.likes > current.likes ? prev : current
  );
  const max3 = technology.reduce((prev, current) =>
    prev.likes > current.likes ? prev : current
  );
  const max4 = fitness.reduce((prev, current) =>
    prev.likes > current.likes ? prev : current
  );
  const max5 = food.reduce((prev, current) =>
    prev.likes > current.likes ? prev : current
  );

  tempArr.push(max1, max2, max3, max4, max5);
  tempArrFinal = tempArr.sort(
    (prev, current) => parseFloat(current.likes) - parseFloat(prev.likes)
  );
            ...
            if (index === 0) {
              return (
                <div className={styles.topPosts_CardPrimary} key={index}>
                  <div className={styles.cardPrimary_Top}>
                    <img
                      className={styles.cardPrimary_Image}
                      src={object.ImageAsset}
                  ...
                  <div className={styles.cardPrimary_Bottom}>
                    <div className={styles.cardPrimary_Bottom1}>
                      <NavLink
                        to={`/article/${object.Title}/${object.CategoryName}`}
                    ...
                    <div className={styles.cardPrimary_Bottom2}>
                      {index + 1}
                    </div>
                  </div>
                </div>
            ...
            if (index !== 0) {
              return (
                <div className={styles.topPosts_CardSecondary} key={index}>
                  <div className={styles.cardSecondary_Left}>
                    <img
                      className={styles.cardSecondary_Image}
                      src={object.ImageAsset}
                      alt="From the article"
                    />
                  </div>
                  <div className={styles.cardSecondary_Right}>
                    <div className={styles.cardSecondary_Right1}>
                      <NavLink
                        to={`/article/${object.Title}/${object.CategoryName}`}
                        className={styles.cardSecondary_Title}
                      >
                        {object.Title}
                      </NavLink>
                      <div>
                        <span className={styles.cardSecondary_CategoryName}>
                          {object.CategoryName}
                        </span>
                        <span className={styles.cardSecondary_PublishedDate}>
                          {" "}
                          / {object.PublishedDate}
                        </span>
                      </div>
                    </div>
                    <div className={styles.cardSecondary_Right2}>
                      {index + 1}
                    </div>
                  </div>
                </div>
